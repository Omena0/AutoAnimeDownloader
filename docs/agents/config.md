...
| `Notifications.Webhooks[].Language` | `language` | `string` | — | Locale used to render the built-in `{{title}}`/`{{message}}` when the body does not supply its own. Empty = keep the daemon default (pt-BR), so a config written by an older release keeps firing in Portuguese exactly as before. Supported: `pt-BR`, `en` |
